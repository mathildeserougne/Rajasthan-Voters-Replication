...
# Liste des contrôles candidats
full_controls_candidates <- c(
  "GP_population", "GP_lit", "GP_sc", "GP_st", "GP_nbvillages",
  "RES00_obc", "RES00_sc", "RES00_st", "RES05_obc", "RES05_sc", "RES05_st",
  ...
  "index_empl_pre_svysample", "index_opinion_pre", "HH_literacy", "HH_OBC"
)

# Liste des contrôles de base
gpcontrols <- c(
  "GP_population", "GP_lit", "GP_sc", "GP_st", "GP_nbvillages",
  "RES00_obc", "RES00_sc", "RES00_st", "RES05_obc", "RES05_sc", "RES05_st"
)
...
incum_dep_vars <- c("INC05_running", "INC05_voteshare", "INCSPOUSE05_running", "INCSPOUSE05_voteshare", "INCOTHER05_running", "INCOTHER05_voteshare")

# Indices de performance
indices <- c("index_empl_svy_0", "index_empl_svy_1", "index_empl_svy_2", "index_empl_svy_3")

# Fonction pour le double lasso
double_lasso_fit <- function(data_df, y_name, d_names, x_candidates, baseline_names = NULL, fe_name = "district", se_cluster = NULL, family = "gaussian") {
  ...
  # Variables dépendantes et de traitement
...
# Fonction pour calculer les effets marginaux
calculate_effects <- function(model, index_name, index_mean, index_sd) {
  coef_names <- names(coef(model))
  int_term <- coef_names[grepl("INT_treatment", coef_names) & grepl(index_name, coef_names)]
  
  effect_average <- coef(model)["INT_treatment"] + coef(model)[int_term] * index_mean
  effect_good <- coef(model)["INT_treatment"] + coef(model)[int_term] * (index_mean + index_sd)
  effect_bad <- coef(model)["INT_treatment"] + coef(model)[int_term] * (index_mean - index_sd)
  
  test_1 <- tryCatch({
    waldtest(model, c(paste0("INT_treatment + ", int_term, " = 0"), paste0(index_name, "=", index_mean)))
  }, error = function(e) NULL)
  
  test_2 <- tryCatch({
    waldtest(model, c(paste0("INT_treatment + ", int_term, " = 0")))
  }, error = function(e) NULL)
...
# Préparation des résultats pour stargazer
...
treat_table <- bind_rows(tidy_list) %>%
  filter(term %in% c("INT_treatment", "TEMP_index", "TEMP_X_anytr_index")) %>%
  mutate(
    dep_var = rep(incum_dep_vars, each = length(indices) * 2)[model_id],
    index = rep(rep(indices, each = 2), length(incum_dep_vars))[model_id],
    RES05_gender = rep(rep(c(0, 1), each = length(indices)), length(incum_dep_vars))[model_id],
...
stargazer(models_list,
          type = "text",
          title = "Table 2: Incumbent and incumbent's family entry depending on incumbent's performance in MGNREGS work provision",
          dep.var.labels.include = FALSE,
          model.numbers = FALSE,
          column.labels = c("Incumbent Runs", "Incumbent Voteshare", "Incumbent Spouse Runs", "Incumbent Spouse Voteshare", "Other Family Member Runs", "Other Family Member Voteshare"),
          covariate.labels = c("Treatment" = "Treatment",
                               "TEMP_index" = "Performance Index",
                               "TEMP_X_anytr_index" = "Treatment*Performance"),
          notes = "The sample includes all 152 GPs which were surveyed and where the incumbent could re-run in 2010: 60 of them had been reserved for women in 2005. Information on voteshare is missing for three GPs.",
          notes.append = TRUE,
          omit.stat = c("ser", "f"),
          style = "default")
