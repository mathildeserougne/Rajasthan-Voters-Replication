...
# ### DONNÉES ET MACROS ###
# Liste baseline de la Table 3
gpcontrols_table3 <- c("GP_population", "GP_lit", "GP_sc", "GP_st", "GP_nbvillages",
                       "RES00_gender", "RES00_obc", "RES00_sc", "RES00_st",
                       "RES10_obc", "RES10_sc", "RES10_st", "RES05_obc", "RES05_sc", "RES05_st")

# Liste complète des contrôles candidats (comme dans votre exemple précédent)
full_controls_candidates <- c(
  ...
  # Ajouts : Économie/Emploi (pré-2010)
  "web_hh_employed2009", "web_persondays_total2009", "log_web_persondays_total2009"
  
  # Ajouts : Indices composites
  #"index_empl_pre_svysample" 
  #,"index_opinion_pre",
  
  # Ajouts : HH (pré-traitement)
  #"HH_literacy", "HH_OBC"
)

# Variables dépendantes de la Table 3
...
double_lasso_fit <- function(data_df, y_name, d_names, x_candidates, baseline_names = NULL, fe_name = "district", se_cluster = NULL, family = "gaussian") {
  # Filtrer les données pour éliminer les NA dans les variables nécessaires
  required_vars <- c(x_candidates, fe_name, y_name, d_names)
  data_df <- data_df[, required_vars, drop = FALSE]
  ...
  if (!is.null(fe_mat)) {
    ...
    if (nrow(X_mat) != nrow(fe_mat)) {
      stop("X_mat et fe_mat n'ont pas le même nombre de lignes.")
    }
  ...
  # Filtrer y_vec et D_mat avec les mêmes lignes que X_mat
...
# Vérifiez les données manquantes dans les variables candidates
...
# Fonction pour calculer les tests de Wald
...
cat("\n==== COMPARAISON CONTROLES (baseline vs double lasso) ====\n")
