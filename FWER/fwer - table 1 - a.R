...
library(multcomp)  # for romano wolf method
...
# LOOP FOR ADJUSTED P VALUES

# extract p-values and apply FWER
get_adjusted_pvalues <- function(models, var_names) {
  all_pvalues <- c()
  for (model in models) {
    coef_table <- summary(model)$coefficients
    # keep just p values and interest variables
