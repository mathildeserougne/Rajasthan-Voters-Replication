...
# But we also add a "whole sample" panel.

# FWER method on Table 2 (Performance), with three panels and only selected models.
...
# Selected indices
selected_indices <- c("index_empl_svy_0", "index_empl_svy_2")

# Estimation of the models for each panel
models_list <- list()
control_means <- numeric(length(incum_dep_vars1) * length(selected_indices) * 3)  # 3 panels
i <- 0

# Panel A: Whole sample (RES05_gender = 0 or 1)
...
# Panel B: Without gender quota (RES05_gender == 0)
for (dep_var in incum_dep_vars1) {
  for (index in selected_indices) {
    i <- i + 1
    control_mean <- data_filtered %>%
...
# Panel C: With gender quota (RES05_gender == 1)
for (dep_var in incum_dep_vars1) {
  for (index in selected_indices) {
    i <- i + 1
    control_mean <- data_filtered %>%
...
# Variables for FWER correction
outregvar2 <- c("INT_treatment", "TEMP_index", "TEMP_X_anytr_index")

# Select models for each panel (only the selected ones)
panel_A_models <- models_list[1:(length(incum_dep_vars1) * length(selected_indices))]
panel_B_models <- models_list[(length(incum_dep_vars1) * length(selected_indices) + 1):(2 * length(incum_dep_vars1) * length(selected_indices))]
panel_C_models <- models_list[(2 * length(incum_dep_vars1) * length(selected_indices) + 1):(3 * length(incum_dep_vars1) * length(selected_indices))]
...
# Implement FWER correction for each panel
pvalues_panel_A_selected <- get_adjusted_pvalues_selected(panel_A_models, outregvar2)
pvalues_panel_B_selected <- get_adjusted_pvalues_selected(panel_B_models, outregvar2)
pvalues_panel_C_selected <- get_adjusted_pvalues_selected(panel_C_models, outregvar2)
...
pvals_A_by_var_selected <- get_pvals_by_var_selected(pvalues_panel_A_selected, outregvar2, length(panel_A_models))
pvals_B_by_var_selected <- get_pvals_by_var_selected(pvalues_panel_B_selected, outregvar2, length(panel_B_models))
pvals_C_by_var_selected <- get_pvals_by_var_selected(pvalues_panel_C_selected, outregvar2, length(panel_C_models))

# Function to print results (with both p-values)
print_selected_results <- function(models, var_names, pvalues_by_var, panel_name, control_means, indices, incum_dep_vars1) {
  cat("\n\n======================================================================\n")
  cat(panel_name, "\n")
  cat("======================================================================\n\n")
  ...
  for (i in 1:length(models)) {
    ...
    if (!is.null(model)) {
      ...
      cat("--- Model ", i, ": ", dep_var_name, " (Index: ", index_name, ") ---\n", sep = "")
...
print_selected_results(panel_A_models, outregvar2, pvals_A_by_var_selected,
                       "PANEL A: Whole Sample (Selected Models)",
                       control_means, selected_indices, incum_dep_vars1)

print_selected_results(panel_B_models, outregvar2, pvals_B_by_var_selected,
                       "PANEL B: GP without Gender Quota in 2005 (Selected Models)",
                       control_means, selected_indices, incum_dep_vars1)

print_selected_results(panel_C_models, outregvar2, pvals_C_by_var_selected,
                       "PANEL C: GP with Gender Quota in 2005 (Selected Models)",
                       control_means, selected_indices, incum_dep_vars1)
